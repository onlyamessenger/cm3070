...
Future<dynamic> handleCompleteQuest(final dynamic context, AppWriteClient appwrite) async {
  ...
  try {
    ...
    final CompleteQuest useCase = CompleteQuest(
      progressService: AppWritePlayerQuestProgressService(
        databases: appwrite.databases,
        databaseId: appwrite.databaseId,
      ),
      questService: AppWriteQuestService(databases: appwrite.databases, databaseId: appwrite.databaseId),
      questNodeService: AppWriteQuestNodeService(databases: appwrite.databases, databaseId: appwrite.databaseId),
      awardXp: awardXp,
      sectionService: AppWriteReadinessSectionService(databases: appwrite.databases, databaseId: appwrite.databaseId),
      logger: logger,
    );

    final CompleteQuestResult result = await useCase.execute(CompleteQuestInput(progressId: progressId));
    ...
    logger.error('Complete quest error: $e');
